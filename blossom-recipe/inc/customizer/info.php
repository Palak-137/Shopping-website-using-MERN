...
function blossom_recipe_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info = '<p>';
	$theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'blossom-recipe' ),  '<a href="' . esc_url( 'https://demo.blossomthemes.com/blossom-recipe/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
	...
	$manager->add_section(
		new Blossom_Recipe_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => 'https://blossomthemes.com/downloads/blossom-recipe-pro/',
			)
		)
	);
}
