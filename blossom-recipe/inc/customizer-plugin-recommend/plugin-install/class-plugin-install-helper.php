		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				case 'install':
					...
					$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="install-now install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="Install ' . $slug . '">' . __( 'Install and activate', 'blossom-recipe' ) . '</a>';
					break;
				...
				case 'activate':					
					...
					$button .= '<a data-slug="' . esc_attr( $slug ) . '" class="activate button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . __( 'Activate', 'blossom-recipe' ) . '</a>';
					break;
			}// End switch().
			$button .= '</div>';
		}// End if().
